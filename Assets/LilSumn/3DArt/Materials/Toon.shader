...
Shader "Custom/Toon" {
   Properties {
    ...
    float3 combinedLight = (ambientLight + diffuseReflection);
    if (_OutlineThickness > 0)
    	combinedLight *= outlineStrength;
    combinedLight += specularReflection;
    float3 wtf = (combinedLight-_WTF,combinedLight-_WTF,combinedLight-_WTF);
    return float4(wtf, 1.0) + (tex2D(_MainTex, input.uv)); // DELETE LINE COMMENTS & ';' TO ENABLE TEXTURE
